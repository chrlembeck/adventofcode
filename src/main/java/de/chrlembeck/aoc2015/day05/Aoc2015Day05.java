...
public class Aoc2015Day05 extends AbstractAocBase {
    ...
    private final static Pattern AT_LEAST_THREE_VOWELS = Pattern.compile(".*([aeiou].*){3}.*");

    private final static Pattern REPEAT_ONE_BETWEEN = Pattern.compile(".*(\\w)\\w\\1.*");

    private final static Pattern REPEATING_CHARS = Pattern.compile(".*(\\w)\\1.*");

    private final static Pattern REPEATING_PAIR = Pattern.compile(".*([a-z]{2}).*\\1.*");

    private final static Pattern FORBIDDEN_WORDS = Pattern.compile(".*(ab|cd|pq|xy).*");
